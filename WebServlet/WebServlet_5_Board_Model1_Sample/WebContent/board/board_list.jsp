...
<body>
	...
	<%
		...
		if(ps == null || ps.trim().equals("")){
			//default 값 설정
			ps = "5"; //100건의 글이 있으면 기본적으로 5개씩 묶겠다 
		}
	
		if(cp == null || cp.trim().equals("")){
			//default 값 설정
			cp = "1"; // 그럼 위 if 절에서 한 번에 5건씩 잡는다고 하면, 몇 번째 페이지부터 출력? 1번! 이런 뜻
					// 즉 1번부터 5번 글
		}
				...
				<!-- 리스트에서 가장 중요한 건 목록을 만드는 일이다 -->
					<tr onmouseover="this.style.backgroundColor='gray'" onmouseout="this.style.backgroundColor='white'">
						...
						원본글은 depth가 0이므로 포이치문이 돌지 않는다 -->
							...
							<!-- 상세보기를 할 때 글 번호, 현재체이지 번호, 페이지 사이즈를 여전히 가지고 있어야 돌아올 때 자리를 찾는다 -->
							<a href="board_content.jsp?idx=${board.idx}&cp=${cpage}&ps=${pagesize}">
								<!-- 제목이 길 경우 잘라서 보여주는 조건문 -->
								<c:choose>
