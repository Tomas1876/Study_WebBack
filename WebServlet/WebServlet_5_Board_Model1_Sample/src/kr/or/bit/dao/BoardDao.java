...
public class BoardDao {
	...
	// refer값은 글번호와 같을 필요는 없지만 중복값이 있지도 않다
	private int getMaxRefer() {
		...
		int refer_max=0;
		try {
			conn = ds.getConnection(); // 미리 만들어둔 커넥션 빌려쓰기
			
			// jspboard 테이블의 refer 컬럼의 데이터 중 최대값을 찾는다
	...
	public Board getContent(int idx) {
		...
		try {
			conn = ds.getConnection();
			// 이렇게 *를 사용하면 뭘 select하는지 알기 어려우니 다 명시하자
	...
	public int deleteOk(String idx , String pwd) {
		...
		원본글 (답글이 있는 경우)
		case 1:  원본글이 삭제시 답변글 있으면 다 삭제 (같은 refer delete) - 개발자 속편한 경우
